...
import type { ModifyMilestoneAuthResponse } from '@/libs/type/client';
...
export default async function handleModifyMileStone(
  req: NextApiRequest,
  res: NextApiResponse<ModifyMilestoneAuthResponse>,
) {
  ...
  const result = await server.putModifyMilestone({
