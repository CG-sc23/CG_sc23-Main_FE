...
export default async function handleCreateMilestone(
  req: NextApiRequest,
  res: NextApiResponse<CreateMilestoneAuthResponse>,
) {
  ...
  const body = req.body as CreateMileStoneAuthTokenAndBody['body'];
  if (!body.subject) return res.status(400).end();
  if (!body.due_date) return res.status(400).end();

  const result = await server.postCreateMileStone({
