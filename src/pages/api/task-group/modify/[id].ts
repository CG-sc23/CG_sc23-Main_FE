...
import type { ModifyTaskGroupAuthResponse } from '@/libs/type/client';
...
export default async function handleModifyTaskGroup(
  req: NextApiRequest,
  res: NextApiResponse<ModifyTaskGroupAuthResponse>,
) {
  ...
  const result = await server.putModifyTaskGroup({
