...
export default async function handleCreateTaskGroup(
  req: NextApiRequest,
  res: NextApiResponse<CreateTaskGroupAuthResponse>,
) {
  ...
  const body = req.body as CreateTaskGroupAuthTokenAndBody['body'];
  if (!body.title) return res.status(400).end();
  if (!body.due_date) return res.status(400).end();

  const result = await server.postCreateTaskGroup({
