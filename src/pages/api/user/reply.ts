...
import type { ReplyProjectInviteResponse } from '@/libs/type/client';
...
export default async function handleReplyInvitee(
  req: NextApiRequest,
  res: NextApiResponse<ReplyProjectInviteResponse>,
) {
  if (req.method !== 'POST') return res.status(405).end();

  if (!req.headers.authorization) return res.status(401).end();
  const authHeader = req.headers.authorization as AuthHeader;
  const result = await server.postReplyProjectInvite({
