...
import type { GetProjectInviteForInviterResponse } from '@/libs/type/client';
...
export default async function handleInviter(
  req: NextApiRequest,
  res: NextApiResponse<GetProjectInviteForInviterResponse>,
) {
  if (req.method !== 'GET') return res.status(405).end();

  if (!req.headers.authorization) return res.status(401).end();
  const authHeader = req.headers.authorization as AuthHeader;
  const result = await server.getProjectInviteForInviter({
